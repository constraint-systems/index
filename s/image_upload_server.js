...
app.post('/post', upload.single('image'), function(req, res, next) {
  ...
  fs.writeFileSync(
    path.join(__dirname, '../static/images/' + new_filename),
    req.file.buffer
  )
...
app.get('/image_list', (req, res) => {
  let dir = path.join(__dirname, '../static/images')
  fs.readdir(dir, (err, files) => {
    ...
    let html = files
      .map(filename => {
        return `<div><img src="http://localhost:1313/images/${filename}" /><div>/images/${filename}</div></div><hr />`
      })
      .join('\n')
